...
def test_change_rules(setup_game, rule_name, new_value, expect_success):
    ...
    if expect_success:
        ...
        assert (
            getattr(game, rule_name) == new_value
        ), f"Rule {rule_name} don't changet on {new_value}."
    else:
...
def test_deal_initial_cards(setup_game):
    ...
    assert len(game.dealer.hand) == 1, "Dealer did not receive their visible card."
    assert game.dealer.hide_card is not None, "Dealer did not receive a hidden card."
...
def test_settle_bets(setup_game, player_score, dealer_score, expected_result):
    ...
    if expected_result == "win":
        ...
        assert (
            player.chips == initial_chips - bet_amount
